...
def main(device_ids, test_ids, flag='data'):
    ...
    save_file_name = '{}_dv{}_channel{}.pth'.format(flag, device_flag, test_flag)
    ...
    for device_id in device_ids:
        for test_id in test_ids:
            filename = './Dataset/MP/all/' \
                       'A_No_{}_19dBm_MP{}.mat'.format(device_id + 1, test_id)
            filename = './Dataset/MP/all/' \
                       'A_No_{}_19dBm_MP{}.mat'.format(device_id + 1, test_id)
            ...
            for segment_id in range(len(segment_index)):
                ...
                if is_get_correct_freq_offset:
                    process_segment_fo, freq_offset2 = synchronization_fine2(process_segment.clone().detach(),
                                                                             max_index)
