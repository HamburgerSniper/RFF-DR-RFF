...
class Trainer(mt.BaseTrainer, Confs):
    def __init__(
            ...
            lamda=1.0, alpha=1.0, beta=1.0, epsilon=0.0, is_NS=False, is_HP=True, is_swap=True, is_FIR=False,
            is_TS=False):
        ...
        if self.is_HP and self.is_NS:
            self.models['F'] = NS_CLF_L2Softmax(in_channels=self.nc, out_channels=self.class_num, d1=8, d2=self.d2 - 2,
                                                z_dim=self.z_dim).to(self.device)
        elif (not self.is_HP) and self.is_NS:
            self.models['F'] = NS_CLF_Softmax(in_channels=self.nc, out_channels=self.class_num, d1=8, d2=self.d2 - 2,
                                              z_dim=self.z_dim).to(self.device)
        elif self.is_HP and (not self.is_NS):
            self.models['F'] = CLF_L2Softmax(in_channels=self.nc, out_channels=self.class_num, d1=self.d1, d2=self.d2,
                                             z_dim=self.z_dim).to(self.device)
        else:
        ...
        self.train_sets['train_y'] = RFdataset(device_ids=range(45), test_ids=[1, 2, 3, 4], rand_max_SNR=self.train_snr,
                                               is_FIR=self.is_FIR)
        self.train_sets['train_c'] = RFdataset(device_ids=range(45), test_ids=[1, 2, 3, 4], rand_max_SNR=self.train_snr,
                                               is_FIR=self.is_FIR)
    ...
    def eval(self, epoch, eval_model=['F'], eval_dataset=None, mode=None, is_record=True):
        ...
        for model in eval_model:
            ...
            distance_hist_plot(intra_dist, inter_dist,
                               filename='./plots/{}_{}_dist_hist.png'.format(self.flag, eval_dataset))
